...
void createSkeleton(RenderWindow& window, int groundHeight, int posX) {
    Enemy* skeleton = new Enemy(false, 100, 2, initializeInfo(165, 40, 1.5, Vector2i(128, 128), skeletonHitbox, skeletonAttackBox, 2, 5, groundHeight - skeletonHitbox.h + 20, posX));
    ...
    textures.push_back(std::make_tuple(window.loadTexture("../graphics/enemies/skeleton/Skeleton_Warrior/Attack_3.png"), 0, 4, 4, 8));
